...
def upgrade():
    ...
    op.add_column('clients', sa.Column('alerts_disabled_at', sa.Integer(), nullable=False, server_default='0'))
    op.add_column('clients', sa.Column('last_activity_at', sa.Integer(), nullable=False, server_default='0'))
    op.add_column('clients', sa.Column('last_alert_sent_at', sa.Integer(), nullable=False, server_default='0'))
    op.add_column('clients', sa.Column('last_pm25', sa.Float(), nullable=True))
    op.add_column('clients', sa.Column('num_alerts_sent', sa.Integer(), nullable=False, server_default='0'))
    ...
    op.add_column('zipcodes', sa.Column('max_sensor_distance', sa.Float(), nullable=False, server_default='0'))
    op.add_column('zipcodes', sa.Column('min_sensor_distance', sa.Float(), nullable=False, server_default='0'))
    op.add_column('zipcodes', sa.Column('num_sensors', sa.Integer(), nullable=False, server_default='0'))
    op.add_column('zipcodes', sa.Column('pm25', sa.Float(), nullable=False, server_default='0'))
    op.add_column('zipcodes', sa.Column('pm25_updated_at', sa.Integer(), nullable=False, server_default='0'))
    ...
    conn = op.get_bind()
    res = conn.execute(
        "SELECT zipcode_id, client_id, last_executed_at, last_pm25, disabled_at FROM subscriptions ORDER BY disabled_at DESC"
    )
    if res is None:
    ...
    for zipcode_id, client_id, last_executed_at, last_pm25, disabled_at in res.fetchall():
        res = conn.execute(
            "UPDATE clients SET zipcode_id=%s, last_pm25=%s, last_alert_sent_at=%s, alerts_disabled_at=%s WHERE id=%s",
            zipcode_id,
            last_pm25,
            last_executed_at,
            disabled_at,

...
def downgrade():
    ...
    sa.Column('last_executed_at', sa.INTEGER(), autoincrement=False, nullable=False),
    sa.Column('last_pm25', postgresql.DOUBLE_PRECISION(precision=53), autoincrement=False, nullable=True),
    sa.ForeignKeyConstraint(['client_id'], ['clients.id'], name='subscription_client_id_fkey'),
    sa.ForeignKeyConstraint(['zipcode_id'], ['zipcodes.id'], name='subscription_zipcode_id_fkey'),
    sa.PrimaryKeyConstraint('zipcode_id', 'client_id', name='subscriptions_pkey')
    )
    op.create_index('ix_subscriptions_last_executed_at', 'subscriptions', ['last_executed_at'], unique=False)
    op.create_index('ix_subscriptions_disabled_at', 'subscriptions', ['disabled_at'], unique=False)
    ...
    sa.Column('max_sensor_distance', postgresql.DOUBLE_PRECISION(precision=53), autoincrement=False, nullable=False),
    sa.Column('min_sensor_distance', postgresql.DOUBLE_PRECISION(precision=53), autoincrement=False, nullable=False),
    sa.ForeignKeyConstraint(['zipcode_id'], ['zipcodes.id'], name='metrics_zipcodes_fkey'),
